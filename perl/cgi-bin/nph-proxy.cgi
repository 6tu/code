...
#   CGIProxy 2.1beta19
#
#   nph-proxy.cgi-- CGIProxy 2.1: 基于CGI脚本的匿名代理程序.
#     支持 HTTP，HTTPS和FTP协议，可过滤cookie，广告及script,
#     用户自定义目标URL等.
#     支持 Perl 5.
#
#   Copyright (C) 1996, 1998-2008 by James Marshall, james@jmarshall.com
#   All rights reserved.  非商业上自由使用; 商业用途需要准许.
#
#   For the latest, see http://www.jmarshall.com/tools/cgiproxy/
#
#
#   关于匿名浏览的重要提示:
#     为保证更高的匿名程度,请关闭 JavaScript.但是匿名的再好,毕竟不是防弹衣.
#     我最大限度的移除JS,如果您发现还有遗漏的地方,请告诉我. 这个脚本不能修
#     改 PDF 文件或者是第三方档案文档,如果您允许这样的连接,可能降低您的匿
#     名度.如果您发现别的危害匿名的事情, 请告诉我.
#
#
#   设置:
#
...
#       . 如果您在使用 HTTP 或 SSL 代理, 设置 $HTTP_PROXY,
#           $SSL_PROXY, 和 $NO_PROXY 是必须的.  如果这些代理使用认证, 
#           设置 $PROXY_AUTH 和 $SSL_PROXY_AUTH 项.
#       . 如果运行在非443端口的HTTPS服务器上, 设置
#           $RUNNING_ON_SSL_SERVER=1 (否则最好为 '' ).
#
#     选项包括:
#       . 设置 $TEXT_ONLY, $REMOVE_COOKIES, $REMOVE_SCRIPTS, $FILTER_ADS,
#           $HIDE_REFERER, 及 $INSERT_ENTRY_FORM 作为已有.  如果匿名很重要
#           设置$REMOVE_SCRIPTS .
#       . 让用户选择所有设置 (除 $TEXT_ONLY),设 $ALLOW_USER_CONFIG=1.
#       . Flash支持设置 $PROXIFY_SWF=1.
#       . 更改URL的编码格式, 修改 proxy_encode() 和 proxy_decode() 默认值.
#       . 编码 cookies, 修改 cookie_encode() 和 cookie_decode() 默认值.
#       . 使用权限设置 @ALLOWED_SERVERS 和 @BANNED_SERVERS.
#       . 相似的, 你能指定允许和拒绝服务清单用包括 cookies 和 scripts.
#       . 为安全, 可禁止访问私有IP范围, @BANNED_NETWORKS.
#       . If filtering ads, you can customize this with a few settings.
#       . 每页插入自己的HTML,设置 $INSERT_HTML 或者 $INSERT_FILE.
#       . 最后(As a last resort), 如果不能作为NPH运行,设置 $NOT_RUNNING_AS_NPH=1.
#       . 由于未加工负载平衡 among a set of proxies, 设置 @PROXY_GROUP.
#       . 别的设置看用户配置选项.
#       . 如果你使用这个代理造成服务器过载, 请看 "NOTES ON PERFORMANCE(性能)" 选项下面.
#
#     要详细了解,每个选项下面还有说明.
#
#     这代码必须作为一个 non-parsed header (NPH) 脚本安装.
#       在大多WEB服务器中，用"nph-"作为文件名的开始即可
#       它也可以作为非NPH脚本，大多设置 $NOT_RUNNING_AS_NPH 标记.
#       这是没有深思熟虑的. 看这个选项的警示说明.
#
#--------------------------------------------------------------------------
...
# 1 为仅允许HTML数据, 0 为允许所有
# 图片用一个1x1透明GIF, 设置 $RETURN_EMPTY_GIF
# (it can still be circumvented with uuencode, etc.).

$TEXT_ONLY= 0 ;


# 若设为 0,请看下面的 @ALLOWED_COOKIE_SERVERS 和 @BANNED_COOKIE_SERVERS
# 你也能用图片阻止COOKIE,看下面的 $NO_COOKIE_WITH_IMAGE

$REMOVE_COOKIES= 0 ;


# 删除广泛的JS MINE标记,这也将删除流行的广告!  :)

$REMOVE_SCRIPTS= 0 ;


# 如果设定, 之后过滤跳出图片 @BANNED_IMAGE_URL_PATTERNS,
# 也移除附带图片的COOKIE, 如果设置了 $NO_COOKIE_WITH_IMAGE的话
# 如果要移除普通的广告,请设置上面的 $REMOVE_SCRIPTS=1 .
...
# 修改默认的  mini_start_form() 代码.
# 如果想在这个表单插入其它的代码,请看$INSERT_HTML 和 $INSERT_FILE below.
...
# 编码URLS.
# 在2.1beta9版本之后, don't call these functions directly.  Rather,
#   用 wrap_proxy_encode() 和 wrap_proxy_decode() 代替.
# 提示: 若修改了默认值, 同时 $PROXIFY_SCRIPTS 也设置,则必须修改 $ENCODE_DECODE_BLOCK_IN_JS
...
# 可能是用JS编码URL相关的 $PROXIFY_SCRIPTS 和 $ENCODE_DECODE_BLOCK_IN_JS
...
# 访问的黑白域名控制, 方法: 'www.example.com' 变成 '^www\.example\.com$'.

@ALLOWED_SERVERS= () ;
@BANNED_SERVERS= () ;


# 访问的黑白IP控制,存在域名反解析,下面的设置是不许访问内网
@BANNED_NETWORKS= ('172', '192.168', '172', '10', '169.254', '244.0.0') ;


# 有选择性的许可COOKIE的域名.
# 用 @ALLOWED_COOKIE_SERVERS 和 @BANNED_COOKIE_SERVERS
...
# 拒绝 cookies 用图片返回 images.  This actually prevents
...
# 不许可的图片站点, at http://www.junkbusters.com .
...
# 若设置$TEXT_ONLY,则设置这项返回空白 GIF.
$RETURN_EMPTY_GIF= 0 ;



# 用户是否使用虚假的 IP使用本代理, 针对命令行工具, 返回 "1" or "0". 
$USER_IP_ADDRESS_TEST= '' ;


# 与 @ALLOWED_SERVERS 和 @BANNED_SERVERS 相关的
...
# 是否支持 NPH 脚本, 不支持则去掉文件名的 "nph-" 标记.
# 服务器必须支持 "Status:" CGI 响应头.
...
# @PROXY_GROUP() 可能太多的用在服务器负载平衡上,自由为用户选择一个代理
#   It was inspired in part by Mike Reiter's and Avi
#   Rubin's "Crowds", at http://www.research.att.com/projects/crowds/ .
# 确保URL编码必须相同
#   Same goes for pack_flags() and unpack_flags() if you modified them,
...
# JS编码URL的 $ENCODE_DECODE_BLOCK_IN_JS also.
# If this is set, and if you modify proxy_encode() and proxy_decode(), 
