...
foreach($temp as $v){
    ...
    if(strpos($v, '.sha512')){
        ...
        $sha512_dir = $date_dir . '/sha512';
        if(!is_dir($sha512_dir)){
            mkdir(iconv("UTF-8", "GBK", $sha512_dir), 0777, true);
        }
        if(!is_dir($log_dir)){
