 ...
 * 脚本下载: http://dl.kn007.net/directlink/yhtz.php
 * 
 * 2016-03-22更新：
 * 1.更新MySQL模块为MySQLi模块（MySQL模块已被废弃）
 * 2.支持无端口测试连接MySQL（适用于禁用端口，使用socket访问的MySQL）
 * 3.修改了$_SERVER[PHP_SELF]为$_SERVER['PHP_SELF']
 * 4.替换了eregi过期函数为preg_match -i
  * ----------------------------------------------------
 */
...
//ajax调用实时刷新
if ($_GET['act'] == "rt"){
	$arr=array('nic'=>"$nic", 'os_info'=>"$os_info", 
	'totalSpace'=>"$dt",'useSpace'=>"$du",'freeSpace'=>"$df",'hdPercent'=>"$hdPercent",'barhdPercent'=>"$hdPercent%",
	'TotalMemory'=>"$mt",'UsedMemory'=>"$mu",'FreeMemory'=>"$mf",'CachedMemory'=>"$mc",'Buffers'=>"$mb",
	'TotalSwap'=>"$st",'swapUsed'=>"$su",'swapFree'=>"$sf",'loadAvg'=>"$load",
	'uptime'=>"$uptime",'freetime'=>"$freetime",'bjtime'=>"$bjtime",'stime'=>"$stime",
	...
	'NetOut2'=>"$NetOut[3]",'NetOut3'=>"$NetOut[3]",'NetOut4'=>"$NetOut[4]",'NetOut5'=>"$NetOut[5]",
	'NetOut6'=>"$NetOut[6]",'NetOut7'=>"$NetOut[7]",'NetOut8'=>"$NetOut[8]",'NetOut9'=>"$NetOut[9]",'NetOut10'=>"$NetOut[10]",
	'NetInput2'=>"$NetInput[3]",'NetInput3'=>"$NetInput[3]",'NetInput4'=>"$NetInput[4]",'NetInput5'=>"$NetInput[5]",
	'NetInput6'=>"$NetInput[6]",'NetInput7'=>"$NetInput[7]",'NetInput8'=>"$NetInput[8]",'NetInput9'=>"$NetInput[9]",'NetInput10'=>"$NetInput[10]",
