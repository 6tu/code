...
foreach($sites as $key => $value){
    ...
    # $json = get_json($value . $query_string); # 判断在线状态
    $json = 'nic';
	...
	$sites_json .= '    $.getJSON("' . $value.$query_string . '", displayData_'.$key.")\r\n";
        $ajax_add_1 .= change_key($key, js_add_1());
        $ajax_add = change_key($key, js_add_2());
        $ajax_add_2 .= str_replace('dataJSON.'.$key.'_', 'dataJSON.', $ajax_add);
        $span_add_1 = str_replace('[' , $key.' [' , html_span_add());
	$span_add .= change_key($key, $span_add_1);
        }
    }
...
if(isset($_GET['act']) && $_GET['act'] == "rt"){
    ...
    # $json=json_encode(jsonstr2array($json_all));
...
function change_key($sites_name, $str){
    ...
    $key2 = array(
		...
		'function displayData_' . $sites_name . '(',
        );
...
function js_static_2($sites_json){
    $js_2_1 =
    <<<EOF
...
EOF;
    ...
    return $js_2_1 . $sites_json .$js_2_3;
    }
